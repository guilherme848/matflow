...
import { Search, Paperclip, Smile, Mic, Send, Zap, Volume2 } from "lucide-react";
          ...
          <div className="flex gap-0 border-b border-border px-3">
            {filtros.map(f => (
              <button
                ...
                className={`px-3 py-2 text-xs font-medium transition-colors relative ${filtroAtivo === f.key ? "text-primary" : "text-muted-foreground hover:text-foreground"}`}
              >
                {f.label}
          ...
          <div className="flex-1 overflow-y-auto">
            {filtered.map((c, i) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 flex gap-3 transition-colors relative animate-card-enter ${conversaAtiva.id === c.id ? "border-l-[3px]" : "border-l-[3px] border-l-transparent hover:bg-secondary"}`}
                style={{
                  animationDelay: `${i * 30}ms`,
                  ...(conversaAtiva.id === c.id ? { borderLeftColor: "#F97316", background: "rgba(249,115,22,0.05)" } : {}),
                }}
              >
                ...
                {c.naoLidas > 0 && (
                  <span className="w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center text-primary-foreground shrink-0 self-center" style={{ background: "#F97316" }}>
                    {c.naoLidas}
                  </span>
                )}
              </button>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="bg-card border-b border-border px-5 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button className="px-3 py-1.5 text-xs rounded-lg border border-border text-foreground hover:bg-secondary transition-colors" onClick={() => toast.success("Conversa transferida!")}>Transferir</button>
              <button className="px-3 py-1.5 text-xs rounded-lg border border-destructive text-destructive hover:bg-destructive/10 transition-colors" onClick={() => toast.success("Conversa fechada!")}>Fechar</button>
              <button className="px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">Ver perfil</button>
              <div className="flex items-center gap-2 ml-3 pl-3 border-l border-border">
                <Zap size={14} className="text-primary" />
                <span className="text-xs font-medium">IA Ativa</span>
                <button
                  onClick={() => setIaAtiva(!iaAtiva)}
                  className={`w-9 h-5 rounded-full transition-colors relative ${iaAtiva ? "bg-success" : "bg-border"}`}
                >
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-3 bg-background">
            {mensagensJoao.map((msg) => {
              if (msg.remetente === "sistema") {
                return (
                  <div key={msg.id} className="flex justify-center">
                    <span className="badge-info text-[10px]">🤖 {msg.texto}</span>
                  </div>
                );
              }
              ...
              return (
                <div key={msg.id} className={`flex ${isCliente ? "justify-start" : "justify-end"}`}>
                  <div
                    className={`max-w-[420px] px-4 py-2.5 text-sm animate-card-enter ${
                      isCliente
                        ? "bg-card border border-border rounded-[0_16px_16px_16px]"
                        : isBot
                        ? "bg-card border border-info/30 rounded-[16px_0_16px_16px]"
                  ...
                  >
                    {isBot && <span className="text-[10px] font-semibold text-info block mb-1">🤖 Bot IA</span>}
                    {msg.audio ? (
                      <div className="flex items-center gap-2 text-muted-foreground">
                        <Volume2 size={16} />
                        <div className="w-24 h-1 bg-border rounded-full" />
                        <span className="text-xs">{msg.duracao}</span>
                      </div>
                    ) : (
                      <p>{msg.texto}</p>
                    )}
                    <span className="text-[10px] text-muted-foreground mt-1 block text-right">{msg.hora}</span>
                  </div>
                </div>
              );
            })}
          ...
          {/* AI Suggestion */}
          <div className="mx-5 mb-2 card-matflow border-l-[3px] animate-card-enter" style={{ borderLeftColor: "#F97316" }}>
            ...
            <div className="flex gap-2">
              ...
              <button className="px-3 py-1 text-xs text-muted-foreground hover:text-foreground transition-colors">Ignorar</button>
            </div>
          </div>

          {/* Input bar */}
          <div className="bg-card border-t border-border px-5 py-3 flex items-center gap-3">
            ...
            <button className="w-10 h-10 rounded-full flex items-center justify-center text-primary-foreground transition-all duration-[120ms] hover:scale-105 active:scale-95 shrink-0" style={{ background: "#F97316" }}>
              <Send size={18} />
            </button>
          </div>
        </div>
        ...
        <div className="w-[300px] bg-card border-l border-border overflow-y-auto shrink-0 hidden xl:block">
          ...
          <div className="p-5 space-y-3 border-b border-border">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-foreground">há 12 dias</span>
            </div>
            <div className="flex justify-between text-sm">
          ...
          <div className="p-5 border-b border-border">
            ...
            <div className="flex flex-wrap gap-1.5">
              <span className="badge-neutral">Construtor</span>
              <span className="badge-attention">Alto Valor</span>
              <span className="badge-success">Recorrente</span>
            </div>
          </div>
          ...
          <div className="p-5 border-b border-border">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="p-2.5 rounded-lg bg-secondary text-sm">
